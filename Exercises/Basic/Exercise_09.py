...
# הסיבה שאני מחלק את המערך לשתיים כי בבדיקה אני בודק מספר מהצחלה ומספר מהסוף
...
if(number.isnumeric() == True):
    ...
    for Start_Counter in range(0,int(len)):
        End_Counter = End_Counter - 1
# בודק האם האןת הרשונה מההתחלה ומהסוף אותו דבר וכך גם השני מההצחלה ומהסוף
